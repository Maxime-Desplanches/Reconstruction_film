% function [S,ixedge,ixtheta] = variogram(x,y,varargin)
...
if params.anisotropy 
    nrthetaedges = floor(360/(params.thetastep))+1;
...
%     theta(I) = 0;
    ...
    thetaedges = linspace(-params.thetastep/2,2*pi-params.thetastep/2,nrthetaedges);
...
if params.plotit
    ...
    if ~params.anisotropy
        plot(S.distance,S.val,marker,'MarkerSize',13);
        axis([0 params.maxdist 0 max(S.val)*1.1]);
        hold on
        plot(xlim(),[var(y) var(y)],'-r','LineWidth',2)
        xlabel('h');
        ylabel('\gamma (h)');
